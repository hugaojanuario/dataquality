...
def create_engine(model: AppViewModel) -> QQmlApplicationEngine:
    ...
    if not engine.rootObjects():
        raise RuntimeError('Não foi possível carregar a interface QML do Sincro.')
    return engine
...
def launch_gui(*, demo: bool = False, project: str = '', screenshots: str = '') -> int:
    ...
    application.setOrganizationName('Sincro')
    application.setApplicationName('Sincro')
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Sincro · desktop Qt Quick')
