...
class AuditApp(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title('Sincro — auditoria de migração')
        ...
        self.profile, self.key_env = tk.StringVar(value='balanced'), tk.StringVar(value='SINCRO_EVIDENCE_KEY')
    ...
    def _suggest(self):
        if not all(k in self.discovery for k in ('source', 'target')):
            messagebox.showerror('Sincro', 'Descubra os dois bancos primeiro.')
        elif self.editor.get('1.0', 'end').strip():
            messagebox.showinfo('Sincro', 'O editor contém revisão. Edite-a ou limpe o texto para gerar novamente.')
        else:
    ...
    def _exclude(self):
        ...
        if not selected or not selected[0].startswith('source:'):
            messagebox.showinfo('Sincro', 'Selecione uma tabela da origem.')
            return
        ...
        try:
            ...
            messagebox.showerror('Sincro', 'Gere sugestões no Mapeamento e tente novamente.')
