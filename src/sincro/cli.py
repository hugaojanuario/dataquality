...
from sincro.config import load_config
from sincro.package_validator import run_package_validation
from sincro.report import write_html_report
from sincro.validator import run_validation


def build_parser() -> argparse.ArgumentParser:
    """Cria os comandos públicos do Sincro."""
    parser = argparse.ArgumentParser(prog="sincro", description="Validador de migrações.")
    ...
    from sincro.audit.cli import add_commands
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == 'audit':
        from sincro.audit.cli import run
        return run(args)
    if args.command == 'gui-legacy':
        from sincro.gui import SincroApp
        SincroApp().mainloop()
        return 0
    if args.command == 'gui-audit-legacy':
        from sincro.audit.gui import AuditApp
        AuditApp().mainloop()
        return 0
    if args.command == "gui":
        if args.screenshots and not args.demo:
            ...
            return 2
        from sincro.desktop.app import launch_gui
